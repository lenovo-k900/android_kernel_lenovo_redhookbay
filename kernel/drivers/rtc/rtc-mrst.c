...
static int mrst_read_alarm(struct device *dev, struct rtc_wkalrm *t)
{
	...
	printk(KERN_ALERT" rtc mrst_read_alarm hrs=%d,min=%d,sec=%d,rtc_control=%02x \r\n",t->time.tm_hour,t->time.tm_min,t->time.tm_sec,rtc_control);
...
static const char *wake_source_type[] = {
    ...
    "androidboot.bootreason=lowbat",                                     //low bat
    "androidboot.bootreason=real_reset",                                 //reset
    "androidboot.bootreason=cold_boot",                                  //cold boot
...
static char seace_valu[]={"androidboot.wakesrc"};
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	if (!strncmp(name,"androidboot.wakesrc",12))
	{
	    char *value = strchr(name, '=');
	    ivalue = simple_strtol(value+1, &endptr, 10);
            if (ivalue < 10)
	    {
                strlcpy(wakesrc, wake_source_type[ivalue], sizeof(wakesrc));
	    }else if(ivalue == 10)
	    {
	        strlcpy(wakesrc, "androidboot.bootreason=sec_wdt_reset", sizeof(wakesrc));
	    }else
            {
	        strlcpy(wakesrc, "androidboot.bootreason=unknown_reason", sizeof(wakesrc));
	    }
	}else
	{
...
{
	alarm_sysfs_add();	//andy add for power off alarm
