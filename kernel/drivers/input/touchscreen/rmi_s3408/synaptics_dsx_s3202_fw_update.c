...
#define FW_IMAGE_NAME "synaptics_fw_s3202.img"
...
static int fwu_check_version(void)
{
	...
	retval = fwu->config_data[3] - config_id[3];
...
static int fwu_do_reflash(void)
{
	...
	dev_dbg(&fwu->rmi4_data->i2c_client->dev,
			...
			__func__);
	printk("wyh debug :%s delay 5 second.\n",__func__);
	msleep(5000);
...
#if 0
	fwu_read_f34_flash_status();
	printk("wyh debug : %s  read f34 flash.status %x.\n",__func__,fwu->flash_control.status);
	printk("wyh debug : %s  read f34 flash.command %x.\n",__func__,fwu->flash_control.command);
	printk("wyh debug : %s  read f34 flash.program_enabled %x.\n",__func__,fwu->flash_control.program_enabled);
#endif
...
static int fwu_start_reflash(void)
{
	...
	if(retval > 0){
		...
		dev_info(&fwu->rmi4_data->i2c_client->dev,"%s:no need to updater firmware.\n",__func__);
...
int synaptics_fw_updater_s3202(unsigned char *fw_data)
{
	int retval;
...
EXPORT_SYMBOL(synaptics_fw_updater_s3202);

static void synaptics_fw_updater_work_s3202(struct work_struct *work)
{
	...
	/*struct synaptics_rmi4_data *rmi4_data =*/
			...
			/*fw_update_work);*/
	retval = synaptics_fw_updater_s3202(fwu->ext_data_source);
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
				"%s: Failed to do reflash\n",
				__func__);
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	INIT_WORK(&rmi4_data->fw_update_work, synaptics_fw_updater_work_s3202);
...
{
	synaptics_rmi4_new_function(RMI_FW_UPDATER_S3202, true,
