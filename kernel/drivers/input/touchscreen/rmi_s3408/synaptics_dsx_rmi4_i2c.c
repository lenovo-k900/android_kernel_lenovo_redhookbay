...
static volatile int key_debug = 0;
static unsigned char config_id[4];
static volatile int aera_2d_disable;
static volatile unsigned long f11_abs_report_time;

#ifdef HALL_SWITCH
#define HALL_GPIO (40)
#endif
/*#define TAG 	"wyh: "*/
/*#define key_debug(fmt, ...) if(key_debug)printk(KERN_DEBUG TAG  fmt, ##__VA_ARGS__)*/
...
static ssize_t synaptics_rmi4_debug_switch_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_debug_switch_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t synaptics_rmi4_device_config_id_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_device_config_id_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
...
#endif
#ifdef PROXIMITY
	__ATTR(proximity_enables, (S_IRUGO | S_IWUSR),
	...
	__ATTR(debug_switch, (S_IRUGO | S_IWUSR),
			synaptics_rmi4_debug_switch_show,
			synaptics_rmi4_debug_switch_store),
	__ATTR(device_config_id, (S_IRUGO | S_IWUSR),
			synaptics_rmi4_device_config_id_show,
			synaptics_rmi4_device_config_id_store),
};
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	key_debug = (unsigned int)val;
	printk("key_debug is %ld.\n",val);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		if(status){
			/*button is pressed*/
			if(time_after(jiffies,(msecs_to_jiffies(20)+f11_abs_report_time)))
				aera_2d_disable = 1;
			if(key_debug > 1)
				printk("wyh %s 2d_disable %d.\n",__func__,aera_2d_disable);
		}else{
			/*button is left*/
			if(key_debug > 1)
				printk("wyh %s 2d_disable %d.\n",__func__,aera_2d_disable);
...
#ifdef TYPE_B_PROTOCOL
		input_mt_slot(rmi4_data->input_dev, rmi4_data->num_of_fingers);
		input_mt_report_slot_state(rmi4_data->input_dev,
				MT_TOOL_FINGER, 1);
#endif
		input_report_key(rmi4_data->input_dev,
		...
		unsigned char *touch_count)
{
	...
	case SYNAPTICS_RMI4_F1A:
		*touch_count += synaptics_rmi4_f1a_report(rmi4_data,
				fhandler);
		break;
#ifdef PROXIMITY
...
static irqreturn_t misc_hall_irq(int irq, void *data)
{
	...
	if(gpio_value){
		/*hall far*/
		if(key_debug == 5)
			printk("wyh report_____________________________ hall far.\n");

		input_event(rmi4_data->input_dev, EV_KEY, KEY_HALL_AWAY, 1);
		input_sync(rmi4_data->input_dev);
		input_event(rmi4_data->input_dev, EV_KEY, KEY_HALL_AWAY, 0);
		input_sync(rmi4_data->input_dev);
	}
	else{
		/*hall near*/
		if(key_debug == 5)
			printk("wyh report____________________________ hall near.\n");
		input_event(rmi4_data->input_dev, EV_KEY, KEY_HALL_NEAR, 1);
		input_sync(rmi4_data->input_dev);
		input_event(rmi4_data->input_dev, EV_KEY, KEY_HALL_NEAR, 0);
		input_sync(rmi4_data->input_dev);
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F34:
				...
				printk("wyh device_config_id 1-2-3-4: %x %x %x %x.\n",config_id[0],
...
static void synaptics_rmi4_detection_work(struct work_struct *work)
{
	...
	if (!list_empty(&exp_fn_list)) {
		list_for_each_entry_safe(exp_fhandler,
				...
				&exp_fn_list,
				link) {
			if(rmi4_data->i2c_client->addr == 0x20){
				if (exp_fhandler->fn_type == RMI_FW_UPDATER_S3202){
					printk("wyh i2c_addr 0x20: clear s3202 fun_init.\n");
				list_del(&exp_fhandler->link);
				kfree(exp_fhandler);
				continue;
				}
			}
			if(rmi4_data->i2c_client->addr == 0x38){
				if (exp_fhandler->fn_type == RMI_FW_UPDATER_S3408){
					printk("wyh i2c_addr 0x38: clear s3408 fun_init.\n");
				list_del(&exp_fhandler->link);
				kfree(exp_fhandler);
				continue;
				}
...
*/
	...
	input_mt_init_slots(rmi4_data->input_dev,
			rmi4_data->num_of_fingers + 1); /* extra f51 finger */
#endif
	...
	set_bit(KEY_HALL_NEAR,rmi4_data->input_dev->keybit);
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_HALL_NEAR);
	set_bit(KEY_HALL_AWAY,rmi4_data->input_dev->keybit);
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_HALL_AWAY);
...
#ifdef HALL_SWITCH
/*wyh hall irq request*/
	rmi4_data->hall_irq = gpio_to_irq(HALL_GPIO);

	retval = request_threaded_irq(rmi4_data->hall_irq, NULL,
			misc_hall_irq,IRQ_TYPE_EDGE_BOTH,
			"misc_hall_irq", rmi4_data);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to create hall irq thread\n",
				__func__);
	}
	
	enable_irq_wake(rmi4_data->hall_irq);
...
err_gpio:
	if (platform_data->gpio_config) 
		platform_data->gpio_config(platform_data->gpio, false);
#ifdef PROXIMITY
	kfree(f51);
#endif
/*
...
static const struct dev_pm_ops synaptics_rmi4_dev_pm_ops = {
	.suspend = NULL,//synaptics_rmi4_suspend,
	.resume  = NULL,//synaptics_rmi4_resume,
};
#endif
