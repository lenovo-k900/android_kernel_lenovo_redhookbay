...
#if 0   // tmode can be determined by devices's protocol,not just rx/tx buffer
	/*
...
static int dw_spi_setup(struct spi_device *spi)
{
	...
	if (chip_info) {
		...
		chip->tmode = chip_info->tmode; /* tmode can be also determined by connected device */
	}
...
static void dw_spi_hw_init(struct dw_spi *dws)
{
	...
	rx_sample_dly = 2;  //3,2,1 work for 20MHz and 25MHz
	dw_writew(dws, DW_SPI_RX_SAMPLE_DLY, rx_sample_dly);
	printk("%s-rx_sample_dly=%d\n",__func__, rx_sample_dly);
