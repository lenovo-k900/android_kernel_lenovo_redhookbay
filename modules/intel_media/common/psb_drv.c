...
extern struct platform_driver r63311_lcd_driver;
#endif
...
static bool intel_mid_get_vbt_data(struct drm_psb_private *dev_priv)
{
	...
	len = strnlen(PanelName, PANEL_NAME_MAX_LEN);
	if (len) {
		strncpy(dev_priv->panel_info.name, PanelName, len);
	...
	pdev = platform_device_alloc(PanelName, -1);
	if (!pdev) {
...
#ifdef CONFIG_R63311_MIPI_VIDEO_MODE
	ret = platform_driver_register(&r63311_lcd_driver);
	if (ret != 0) {
		return ret;
	}
#endif
