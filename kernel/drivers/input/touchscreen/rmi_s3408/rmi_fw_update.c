...
#define FW_IMAGE_NAME "synaptics_fw_s3408.img"
#define FW_IMAGE_NAME_OLD "synaptics_fw_old.img"
#define FW_IMAGE_NAME_TEMP "synaptics_fw_temp.img"

#define CONFIG_ID_OFFSET 0xef00
...
static volatile int update_fw_force = 0;
...
static ssize_t fwu_sysfs_update_fw_force_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t fwu_sysfs_update_fw_force_show(struct device *dev,
...
static struct device_attribute attrs[] = {
	...
	__ATTR(forceupdate, (S_IWUSR | S_IRUGO),
			fwu_sysfs_update_fw_force_show,
			fwu_sysfs_update_fw_force_store),
...
static unsigned int ConvertConfigID(const unsigned char *ptr){
	...
	int retval;
	printk("wyh ptr0-1-2-3-4: %x %x %x %x.\n",ptr[0],
					ptr[1],ptr[2],ptr[3]);
	if(ptr[0]<0x30 || ptr[1]<0x30 || ptr[2]<0x30 || ptr[3]<0x30)
		return 0;
	retval = (unsigned int) ptr[3];
...
{
	int retval;
	unsigned int image_configID;
	unsigned int device_configID;
	...
	if (fwu->ext_data_source) {
		...
		if(update_fw_force){
			retval = request_firmware(&fw_entry, FW_IMAGE_NAME_TEMP,
					&fwu->rmi4_data->i2c_client->dev);
			if (retval != 0) {
				dev_err(&fwu->rmi4_data->i2c_client->dev,
						"%s: Firmware image %s not available\n",
						__func__, FW_IMAGE_NAME_TEMP);
				return -EINVAL;
			}
		}else{
    ...
    image_configID = ConvertConfigID(fw_image+CONFIG_ID_OFFSET);

	printk("image file config id is %d \n",image_configID);
	...
	printk("device config id is %d \n",device_configID);
	//for debug	
	if(image_configID>device_configID || update_fw_force){
		printk("do reflash ++++++++++++++++++++++first time.\n");	
		retval = fwu_do_reflash();
		printk("retval first time %d.\n",retval);
#if 1
		if (retval < 0) {
	//		dev_err(&fwu->rmi4_data->i2c_client->dev,
	//				"%s: Failed to do reflash\n",
	//				__func__);
			retval = request_firmware(&fw_entry, FW_IMAGE_NAME_OLD,
					&fwu->rmi4_data->i2c_client->dev);
			if (retval != 0) {
				dev_err(&fwu->rmi4_data->i2c_client->dev,
						"%s: Firmware image %s not available\n",
						__func__, FW_IMAGE_NAME_OLD);
				return -EINVAL;
			}
			...
			printk("do reflash ++++++++++++++++++++++old firmware.\n");	
			retval = fwu_do_reflash();
			if (retval < 0) {
				dev_err(&fwu->rmi4_data->i2c_client->dev,
						"%s: Failed to do reflash second times \n",
						__func__);
			}
			...
			printk("do reflash ++++++++++++++++++++++new firmware again.\n");	
			retval = fwu_do_reflash();
			if (retval < 0) {
				dev_err(&fwu->rmi4_data->i2c_client->dev,
						"%s: Failed to do reflash the third times \n",
						__func__);
			}
		}
#endif
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	update_fw_force = (unsigned int)val;
	printk("update_fw_force is %d.\n",val);
...
{
	synaptics_rmi4_new_function(RMI_FW_UPDATER_S3408, true,
