...
static void penwell_otg_psc_notify_work(struct work_struct *work)
{
	...
	struct power_supply_charger_cap	chg_cap;
	...
	list_for_each_entry_safe(event, temp, &pnw->chrg_evt_queue, node) {
		...
		chg_cap.chrg_evt = chrg_event;
		chg_cap.chrg_type = event->cap.chrg_type;
		chg_cap.mA = event->cap.mA;
		spin_unlock_irqrestore(&pnw->cap_lock, flags);
		...
		if (chrg_event != -1) {
			...
			power_supply_charger_event(chg_cap);
		}
