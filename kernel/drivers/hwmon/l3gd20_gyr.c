						...
						struct l3gd20_triple *data)
{
	...
	//pr_info("%s: gyro data is %d, %d, %d",__func__, data->x,data->y,data->z);
	input_sync(input);
}
					...
					const struct i2c_device_id *devid)
{
	...
	gyro->resume_state[RES_CTRL_REG3] = ALL_ZEROES | 0xA0;/*Modify H_Lactive bit to 1*/
