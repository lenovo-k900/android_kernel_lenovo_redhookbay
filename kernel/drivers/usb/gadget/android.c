...
static void android_work(struct work_struct *data)
{
	...
	char **uevent_envp = NULL;
    static int prev_status = 0; //0 - Disconnect, 1 - Connected, 2 - Configured
    static int cur_status = 0;  //0 - Disconnect, 1 - Connected, 2 - Configured
	unsigned long flags;
    ...
    if (cdev->config)
        popup = 1;
    if(!dev->sw_connected) //Non-USB
    {
        if(!usb_status)
            popup = 2;
        else if(cur_status == 0 && prev_status == 2)
        {
            pr_err("Penwell-OTG running after android_work\n");
            popup = 2;
        }
    }
    spin_unlock_irqrestore(&cdev->lock, flags);

    pr_err("%s: cdev->config %d, pop %d, usb_status %d, dev->sw_connected %d\n", __func__, cdev->config, popup, usb_status, dev->sw_connected);
