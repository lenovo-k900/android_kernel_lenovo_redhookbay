...
static irqreturn_t arizona_ctrlif_err(int irq, void *data)
{
	 ...
	 */
	dev_err(arizona->dev, "Control interface error FOO\n");
...
static irqreturn_t arizona_irq_thread(int irq, void *data)
{
    ...
    if (ret < 0) {
        dev_err(arizona->dev, "Failed to resume device: %d, disable_depth = %d\n", ret, arizona->dev->power.disable_depth);
        return IRQ_NONE;
    }
	...
	do {
        ...
        if (time_after(jiffies, timeout)) {
			poll = true;
            printk("enter %s, timeout and Fail to clear IRQ\n", __func__);
            break;
        }
	} while (poll);
...
int arizona_irq_init(struct arizona *arizona)
{
	...
	if (ctrlif_error) {
		...
		ret = request_threaded_irq(i, NULL, arizona_ctrlif_err,
					   IRQF_ONESHOT,
					   "Control interface error 123", arizona);
		if (ret != 0) {
			dev_err(arizona->dev,
