...
static struct wake_lock lsm303d_wake_lock;
...
/* Sensitivity */
#define SENSITIVITY_ACC_2G	62.5	/**	ug/LSB	*/
#define SENSITIVITY_ACC_4G	125.5	/**	ug/LSB	*/
...
#define REG_DEF_CLK_CFG		(0x2A)
#define REG_DEF_CLK_SRC		(0x19)
#define REG_DEF_CLK_THS		(0x10)
#define REG_DEF_TIME_LIMIT		(0x20)
#define REG_DEF_TIME_LATENCY		(0x03)
#define REG_DEF_TIME_WINDOW		(0x30)

#define ACC_IRQ_GPIO		60
...
struct lsm303d_status {
	...
	struct mutex lock;
	struct delayed_work input_work_lv;/*liuyx4 Add lv sensor for double click--20130106*/
	struct delayed_work input_work_acc;
	struct delayed_work input_work_mag;

	struct early_suspend    early_suspend;

	struct input_dev *input_dev_lv;/*liuyx4 Add lv sensor for double click--20130106*/
...
static int dc_flag = -1;
void check_change()
{
	dc_flag = 1;
}

static DEFINE_TIMER(lsm303d_timer, check_change, 0, 0);

static void lsm303d_work_handler(struct work_struct *work)
{
	struct lsm303d_status *data = container_of(work, struct lsm303d_status, input_work_lv.work);
//	struct i2c_client *client=data->client;
	...
	if (dc_flag == 1)
	{
		dc_flag = 0;
		pr_info("#########%s\n", __func__);
		input_report_abs(data->input_dev_lv, ABS_X, 8);
		input_report_abs(data->input_dev_lv, ABS_Y, 1);
		input_report_abs(data->input_dev_lv, ABS_Z, 8);

		data->input_dev_lv->absinfo[ABS_X].value = -1;
		data->input_dev_lv->absinfo[ABS_Y].value = -1;
		data->input_dev_lv->absinfo[ABS_Z].value = -1;
		input_sync(data->input_dev_lv);
	}
	else
	{
		pr_info("####%s: invalid interrupt!####\n", __func__);
	}
	//wake_unlock(&lsm303d_wake_lock);
}

static void lsm303d_reschedule_work(struct lsm303d_status *data,
					  unsigned long delay)
{
	unsigned long flags;
	
	pr_info("#########%s\n", __func__);
	spin_lock_irqsave(&data->lock.wait_lock, flags);

	/*
	 * If work is already scheduled then subsequent schedules will not
	 * change the scheduled time that's why we have to cancel it first.
	 */
	__cancel_delayed_work(&data->input_work_lv);
	schedule_delayed_work(&data->input_work_lv, delay);

	spin_unlock_irqrestore(&data->lock.wait_lock, flags);
}

static irqreturn_t lsm303d_interrupt(int vec, void *info)
{
	struct i2c_client *client=(struct i2c_client *)info;
	struct lsm303d_status*data = i2c_get_clientdata(client);

	pr_info("#########%s\n", __func__);
	wake_lock_timeout(&lsm303d_wake_lock, HZ * 2);
	lsm303d_reschedule_work(data, 0);
...
static int lsm303d_lv_device_power_off(struct lsm303d_status *stat)
{
	int err;
	u8 buf[2];
...
static int lsm303d_lv_device_power_on(struct lsm303d_status *stat)
{
	...
	buf[0] = status_registers.cntrl1.address;
	buf[1] = ((ODR_ACC_MASK & LSM303D_ACC_ODR800) |
		((~ODR_ACC_MASK) & status_registers.cntrl1.resume_value));
	err = lsm303d_i2c_write(stat, buf, 1);
	if (err < 0) {
		goto err_resume_state;
		dev_err(&stat->client->dev, "accelerometer suspend "
							"failed: %d\n", err);
	}
	...
	if (err < 0)
		goto err_resume_state;
#endif
	i2c_smbus_write_byte_data(client, REG_CNTRL3_ADDR, 0x40);//0x2A

	atomic_set(&stat->enabled_lv, 1);

	dc_flag = 0;
	mod_timer(&lsm303d_timer, jiffies + HZ);
...
static int lsm303d_acc_disable(struct lsm303d_status *stat)
{
	...
	if (atomic_cmpxchg(&stat->enabled_acc, 1, 0)) {
		...
		if (atomic_read(&stat->enabled_lv))
			atomic_set(&stat->enabled_acc, 0);
		else
...
static int lsm303d_lv_enable(struct lsm303d_status *stat)
{
	int err;

	pr_info("##########%s\n", __func__);
	if (!atomic_cmpxchg(&stat->enabled_lv, 0, 1)) {
		err = lsm303d_lv_device_power_on(stat);
		if (err < 0) {
			atomic_set(&stat->enabled_lv, 0);
			return err;
		}
...
static int lsm303d_lv_disable(struct lsm303d_status *stat)
{
	if (atomic_cmpxchg(&stat->enabled_lv, 1, 0)) {
		if(atomic_read(&stat->enabled_acc))
			atomic_set(&stat->enabled_lv, 0);
		else
			lsm303d_lv_device_power_off(stat);
	}
...
/*Begin:liuyx4 Add lv sensor for double click--20130106*/
static void lsm303d_lv_input_cleanup(struct lsm303d_status *stat)
{
	input_unregister_device(stat->input_dev_lv);
	input_free_device(stat->input_dev_lv);
}
/*End*/
				     	...
				     	const char *buf, size_t size)
{
	...
	if (!interval_us)
		return -EINVAL;
	if (interval_us > 320000)
		interval_us = 320000;
	pr_info("######%s: interval_us = %ld\n", __func__, interval_us);
	...
	if (atomic_read(&stat->enabled_lv))
	{
		pr_info("%s: lv sensor is enabled!\n", __func__);
	}
	else
...
static ssize_t attr_get_enable_lv(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct lsm303d_status *stat = dev_get_drvdata(dev);
	int val = (int)atomic_read(&stat->enabled_lv);
	return sprintf(buf, "%d\n", val);
}
...
static ssize_t attr_set_enable_lv(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t size)
{
	struct lsm303d_status *stat = dev_get_drvdata(dev);
	...
	printk("################## lv enable val = %ld \n", val);

	if (val)
		lsm303d_lv_enable(stat);
	else
		lsm303d_lv_disable(stat);
...
static struct device_attribute attributes[] = {
	...
	__ATTR(enable_device_mag, 0664, attr_get_enable_mag, 
						attr_set_enable_mag),
	__ATTR(enable_device_lv, 0664, attr_get_enable_lv,
						attr_set_enable_lv),
...
/*Begin:liuyx4 Add lv sensor for double click--20130106*/
int lsm303d_lv_input_open(struct input_dev *input)
{
	return 0;
}

void lsm303d_lv_input_close(struct input_dev *dev)
{
}
/*End*/
...
static void lsm303d_acc_report_values(struct lsm303d_status *stat, int *xyz)
{
	...
	//printk("~~~ACC~~~~~~~~~~~~~~~~~~~~~ %d, %d, %d \n", xyz[0], xyz[1], xyz[2]);
	//pr_info("%s: acc data is %d, %d, %d",__func__,xyz[0], xyz[1], xyz[2]);
}
...
static void lsm303d_mag_report_values(struct lsm303d_status *stat, int *xyz)
{
	...
	//printk("~~~~~~~~~~~~~~~~~~~~~MAG~~~ %d, %d, %d \n", xyz[0], xyz[1], xyz[2]);
	//pr_info("%s: mag data is %d, %d, %d",__func__, xyz[0], xyz[1], xyz[2]);
}
...
/*Begin:liuyx4 Add lv sensor for double click--20130106*/
static int lsm303d_lv_input_init(struct lsm303d_status *stat)
{
	int err;
	struct i2c_client *client=stat->client;

	// set gpio function and request irq
	INIT_DELAYED_WORK(&stat->input_work_lv, lsm303d_work_handler);
	stat->input_dev_lv = input_allocate_device();
	if (!stat->input_dev_lv) {
		err = -ENOMEM;
		dev_err(&stat->client->dev, "lenovo virtual "
					"input device allocation failed\n");
		goto err0;
	}

	err = gpio_request(ACC_IRQ_GPIO, "acc irq");
	if (err) {
		printk("acc fail to request irq gpio, return :%d\n", err);
	}

	gpio_direction_input(ACC_IRQ_GPIO);
	client->irq = gpio_to_irq(ACC_IRQ_GPIO);

	wake_lock_init(&lsm303d_wake_lock, WAKE_LOCK_SUSPEND, "lsm303d_wake_lock");
	if (request_irq(client->irq, lsm303d_interrupt, IRQF_DISABLED|IRQ_TYPE_EDGE_RISING,
		LSM303D_DEV_NAME, (void *)client)) {
		printk("%s Could not allocate ACC_INT !\n", __func__);
	}
	enable_irq_wake(client->irq);

	printk("%s interrupt is hooked\n", __func__);

	stat->input_dev_lv->open = lsm303d_lv_input_open;
	stat->input_dev_lv->close = lsm303d_lv_input_close;
	stat->input_dev_lv->name = LSM303D_LV_DEV_NAME;
	stat->input_dev_lv->id.bustype = BUS_I2C;
	stat->input_dev_lv->dev.parent = &stat->client->dev;

	input_set_drvdata(stat->input_dev_lv, stat);

	set_bit(EV_ABS, stat->input_dev_lv->evbit);

	input_set_abs_params(stat->input_dev_lv, ABS_X,
					-ACC_G_MAX_NEG, ACC_G_MAX_POS, FUZZ, FLAT);
	input_set_abs_params(stat->input_dev_lv, ABS_Y,
					-ACC_G_MAX_NEG, ACC_G_MAX_POS, FUZZ, FLAT);
	input_set_abs_params(stat->input_dev_lv, ABS_Z,
					-ACC_G_MAX_NEG, ACC_G_MAX_POS, FUZZ, FLAT);

	err = input_register_device(stat->input_dev_lv);
	if (err) {
		dev_err(&stat->client->dev,
			"unable to register lenovo virtual input device %s\n",
				stat->input_dev_lv->name);
		goto err1;
	}
		...
		const struct i2c_device_id *id)
{
	...
	ret = i2c_smbus_read_byte_data(client,REG_WHOAMI_ADDR);
...
/*Begin:liuyx4 Add lv sensor for double click--20130106*/
	/*Init register for double click*/
	i2c_smbus_write_byte_data(client, CLK_CFG, 0x2A);//0x2A
	//i2c_smbus_write_byte_data(client, CLK_SRC, 0x19);
	i2c_smbus_write_byte_data(client, CLK_THS, 0x35);//0x10~ click threshold
	i2c_smbus_write_byte_data(client, TIME_LIMIT, 0x0B);//high to low time limit
	i2c_smbus_write_byte_data(client, TIME_LATENCY, 0x60);//click to click time latency
	i2c_smbus_write_byte_data(client, TIME_WINDOW, 0xE0);//click to click max last time
/*End*/
...
/*Begin:liuyx4 Add lv sensor for double click--20130106*/
	err = lsm303d_lv_input_init(stat);
	if (err < 0) {
		dev_err(&client->dev, "lenovo virtual input init failed\n");
		goto err_power_off;
	}
/*End*/
...
static int __devexit lsm303d_remove(struct i2c_client *client)
{
	...
	wake_lock_destroy(&lsm303d_wake_lock);
	lsm303d_acc_disable(stat);
	lsm303d_mag_disable(stat);
