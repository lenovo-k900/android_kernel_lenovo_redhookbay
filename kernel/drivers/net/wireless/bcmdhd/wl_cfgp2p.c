...
#if 0
static s32 wl_cfgp2p_register_ndev_internal(struct wl_priv *wl,const char * p2ptype,char * mac);

s32 wl_cfgp2p_register_ndev(struct wl_priv *wl)
{
    /*
         zhang wenjie :
         I need to creat widi interface. So hack this api.
    */
    s32 ret=0;

    uint8 temp_addr[ETHER_ADDR_LEN] = { 0x00, 0x90, 0x4c, 0x33, 0x22, 0x11 };
    WL_ERR(("wenjie in wl_cfgp2p_register_ndev. creat p2p "));
    ret=wl_cfgp2p_register_ndev_internal(wl,"p2p%d",temp_addr) ;
    if(ret) 
    {
         WL_ERR(("Could not allocate wireless device\n"));
         return ret;
    }
    WL_ERR(("wenjie in wl_cfgp2p_register_ndev. creat p2pwidi "));
    temp_addr[5]=0x15;//only one by different.
    ret=wl_cfgp2p_register_ndev_internal(wl,"p2pwidi%d",temp_addr) ;
    if(ret) 
    {    // I dont return error when widi create failure.
         WL_ERR(("Could not allocate wireless device for widi.\n"));
    }
     WL_ERR(("wenjie in wl_cfgp2p_register_ndev. return ok"));
    return 0;
...
s32
wl_cfgp2p_register_ndev_internal(struct wl_priv *wl,const char * p2ptype,char * mac)
#else
...
#endif
{
	...
	if (!(net = alloc_etherdev(sizeof(wl)))) {
        ...
        //memcpy(temp_addr,sizeof temp_addr,mac);
	strcpy(net->name, "p2p%d");
        //strcpy(net->name,p2ptype ); 
