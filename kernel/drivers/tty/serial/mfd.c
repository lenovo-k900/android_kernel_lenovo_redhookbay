...
#define HOST_WAKE_GPIO 33
bool noirq_check=false;
struct wake_lock btwake_gpio_lock;
...
static irqreturn_t wakeup_irq(int irq, void *dev)
{
	...
	struct hsu_port_cfg *cfg = phsu->configs[up->index];
       if (up->index == 0) {
		dev_err(dev, "@@@@@@@@ harry HSU wake up, set pm_flags now btwake_gpio level= %x ",gpio_get_value(HOST_WAKE_GPIO));
        if(noirq_check==true)
        {
            dev_err(dev, "btwake irq missed,need wakelock 1 s ");
            wake_lock_timeout(&btwake_gpio_lock, 1*HZ);
            noirq_check=false;
        }}
...
static int serial_hsu_suspend(struct device *dev)
{
	...
	struct uart_hsu_port *up = pci_get_drvdata(pdev);
     ...
     if(up->index==0)
     {             
          if(gpio_get_value(HOST_WAKE_GPIO)!=0)
          {
              dev_err(dev, "%s, EBUSY hostwake_gpio level =%x \n",__func__,gpio_get_value(HOST_WAKE_GPIO));
              return -EBUSY;
          }
          noirq_check=true;
    }
...
static int __init hsu_pci_init(void)
{
	...
	if (!ret) {
	    ...
	    wake_lock_init(&btwake_gpio_lock, WAKE_LOCK_SUSPEND, "brcm_btwake_lock");
	if (ret) {
...
static void __exit hsu_pci_exit(void)
{
	...
	pci_unregister_driver(&hsu_dma_pci_driver);
	uart_unregister_driver(&serial_hsu_reg);
       wake_lock_destroy(&btwake_gpio_lock);
	hsu_debugfs_remove(phsu);
}
