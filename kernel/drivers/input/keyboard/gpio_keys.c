			...
			size_t size, int enable_wakeup)
{
    ...
    pr_err("%s: set wakeup gpio button %ld, status %d\n",
        __func__, code, enable_wakeup);
...
static int gpio_keys_suspend(struct device *dev)
{
	...
	for (i = 0; i < ddata->n_buttons; i++) {
		struct gpio_button_data *bdata = &ddata->data[i];
        pr_err("%s: check wakeup key %d, may wakeup %d\n", 
            __func__, bdata->button->wakeup, device_may_wakeup(dev));
