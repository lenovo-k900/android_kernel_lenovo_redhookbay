...
static ssize_t proc_flash_led_write (struct file *file, const char __user *buf, size_t nbytes, loff_t *ppos)
{
	...
	if (!strcmp((const char *)string, (const char *)"on")) {
		gpio_set_value(lm3554_gpio_strobe, 1);
	} else if (!strcmp((const char *)string, (const char *)"off")) {
		gpio_set_value(lm3554_gpio_strobe, 0);
	}
...
static int lm3554_hw_strobe(struct i2c_client *client, bool strobe)
{
	...
	//printk("lilp3, %s : strobe = %d, %d\n",__func__,strobe,__LINE__);
	/*
...
static int __devinit lm3554_gpio_init(struct i2c_client *client)
{
	...
	struct lm3554 *flash = to_lm3554(sd);
	struct lm3554_platform_data *pdata = flash->pdata;
	lm3554_gpio_strobe = pdata->gpio_strobe;
	int ret;
				  ...
				  const struct i2c_device_id *id)
{
	...
	proc_create_data("CTP_FLASH_CTRL", S_IFREG | S_IWUGO | S_IWUSR, NULL, &proc_flash_led_operations, NULL);
