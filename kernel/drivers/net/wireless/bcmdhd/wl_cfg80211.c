	...
	u8 *mac, struct station_info *sinfo)
{
	...
	if (wl_get_mode_by_netdev(wl, dev) == WL_MODE_AP) {
		...
		err = wldev_iovar_getbuf(dev, "counters", NULL,0, (char *)&cnt,  sizeof(wl_cnt_t), NULL);
		if (err) {
			WL_ERR(("Could not get counters(%d)\n", err));
			goto get_station_err;
		}
		cnt.version = dtoh16(cnt.version);
		if (cnt.version != WL_CNT_T_VERSION) {
			WL_ERR(("\tIncorrect version of counters struct: expected %d; got %d\n",
				WL_CNT_T_VERSION, cnt.version));
			goto get_station_err;
		}

		sinfo->filled |= STATION_INFO_RX_BYTES;
		sinfo->rx_bytes = dtoh32(cnt.rxbyte);
		sinfo->filled |= STATION_INFO_RX_PACKETS;
		sinfo->rx_packets = dtoh32(cnt.rxframe);
		sinfo->filled |= STATION_INFO_TX_BYTES;
		sinfo->tx_bytes = dtoh32(cnt.txbyte);
		sinfo->filled |= STATION_INFO_TX_PACKETS;
		sinfo->tx_packets = dtoh32(cnt.txframe);
		sinfo->filled |= STATION_INFO_TX_FAILED;
		sinfo->tx_failed = dtoh32(cnt.txfail);
...
static s32 wl_inform_single_bss(struct wl_priv *wl, struct wl_bss_info *bi)
{
	...
	if(!channel) {
		WL_ERR(("cfg80211_inform_bss_frame channel  NULL\n"));
		kfree(notif_bss_info);
		return -EINVAL;
	}
