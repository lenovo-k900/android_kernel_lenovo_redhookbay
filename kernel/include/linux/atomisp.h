...
#define CI_MODE_HDR_MOVIE	0x8001
...
struct WB_coefficient{
	u16 abs_gain_r;
	u16 abs_gain_b;

};
#define HDR_LSC_END 		0x49F7
#define HDR_LSC_START 	0x4800

struct HDR_mode_setting{
	u8 HDR_mode; //0: auto mode;1 : direct mode
	u8 exp_ratio; // 1=>1:1, 2=>1:2;4=>1:4; 8=>1:8; 16=>1:16
};
struct HDR_curve_parm{
	u8 Curve_mode;
	u16 tc_out_noise;
	u16 tc_out_mid;
	u16 tc_out_sat;
...
struct atomisp_HDR_parm{
	...
	#define ATR_ONOFF_SETTING	0x01
	#define LSC_SETTING			0x02
	#define WBC_SETTING 		0x04
	#define HDR_MODE_SETTING	0x08
	#define CURVE_MODE_SETTING	0x10
	#define STATS_DATA_LEN		0x20
	struct HDR_mode_setting HDR_mode;
	u8 ATR_onoff; // 1 on;0 off
	struct HDR_curve_parm Curve_parm; //2:fix; 1:ATR mode
	u8 *LSC_param;
	struct WB_coefficient WBC_parm;
	u8	stats_len_set; //0:16*16; 1:8*8;2:4*4; 3:1*1
	int *sensor_stats;
...
#define ATOMISP_IOC_G_SENSOR_HDR_STATS_DATA \
		_IOWR('v', BASE_VIDIOC_PRIVATE + 63, struct atomisp_HDR_parm)

#define ATOMISP_IOC_S_ISP_HDR_SETTING \
		_IOWR('v', BASE_VIDIOC_PRIVATE + 64, struct atomisp_HDR_parm)

#define ATOMISP_IOC_G_ANR \
		_IOR('v', BASE_VIDIOC_PRIVATE + 66, struct atomisp_anr_config)
#define ATOMISP_IOC_S_ANR \
		_IOW('v', BASE_VIDIOC_PRIVATE + 67, struct atomisp_anr_config)


#define ATOMISP_IOC_CONFIG_G_XNR \
	_IOR('v', BASE_VIDIOC_PRIVATE + 68, struct atomisp_xnr_config)
#define ATOMISP_IOC_CONFIG_S_XNR \
	_IOW('v', BASE_VIDIOC_PRIVATE + 69, struct atomisp_xnr_config)
