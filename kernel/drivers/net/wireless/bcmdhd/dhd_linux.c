...
/*
//gaofeng add for wifi disable SGI (short guard interval)

				int tx = 1;
				bcm_mkiovar("sgi_tx", (char *)&tx, 4, iovbuf, sizeof(iovbuf));
				if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
					sizeof(iovbuf), TRUE, 0)) < 0) {
					DHD_ERROR(("%s set sgi_tx for 1 failed  %d\n", __FUNCTION__, ret));
				}
				else {
					 DHD_ERROR(("%s set sgi_tx succeeded\n", __FUNCTION__));
				}
				 int rx = 1;
				 bcm_mkiovar("sgi_rx", (char *)&rx, 4, iovbuf, sizeof(iovbuf));
				if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
					sizeof(iovbuf), TRUE, 0)) < 0) {
					DHD_ERROR(("%s set sgi_rx for 1 failed  %d\n", __FUNCTION__, ret));
				}
				else {
					DHD_ERROR(("%s set sgi_rx succeeded\n", __FUNCTION__));
				}
